...
import com.example.ui.components.CircleAsyncImage
...
@Composable
fun NewsItem(uiModel: NewsItemUiModel, onClick: () -> Unit) {
    ...
    Card(
        ...
        Row {
            CircleAsyncImage(uiModel.imageUrl, modifier = Modifier.fillMaxHeight())

            Column(modifier = Modifier.padding(start = space.space10, top = space.space4).fillMaxWidth()) {
                ...
                Row(modifier = Modifier.padding(bottom = space.space2)) {
                    Text(uiModel.authorName, style = typography.textSmall, color = color.contentStroke)
